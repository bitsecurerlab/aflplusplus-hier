...
EXP_ST void setup_shm(void) {
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE * N_COV_METRICS, IPC_CREAT | IPC_EXCL | 0600);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  memset(trace_bits, 0, MAP_SIZE * N_COV_METRICS);
  MEM_BARRIER();
...
static void check_map_coverage(void) {
  ...
  for(m = 0; m < N_COV_METRICS; m++){
    if (count_bytes(trace_bits + MAP_SIZE * m) < 100) return;

    for (i = (1 << (MAP_SIZE_POW2 - 1)); i < MAP_SIZE; i++)
      if (trace_bits[i + MAP_SIZE * m]) return;
  }
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  perf_score = MAX(perf_score / 5, 10);
  // OKF("%d", perf_score);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, execs_done, cycles_done, cur_path, paths_total, "
                     "path_w_cov, pending_total, pending_favs, map_size, unique_crashes, "
...
int main(int argc, char** argv) {
  ...
  u64 prev_log_time = 0, cur_log_time;
  ...
  while (1) {
    ...
    if (!queue_cur) {
      ...
      show_stats();
      cur_log_time = get_cur_time();
      if (cur_log_time - prev_log_time > 10 * 60){
        if (not_on_tty) {
            ACTF("Entering cycle %llu.", queue_cycle);
            fflush(stdout);
          }
