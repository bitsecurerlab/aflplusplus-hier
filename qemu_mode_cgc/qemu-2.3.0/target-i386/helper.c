...
static target_ulong calling_context = 0; 

// #define N_CC 256
// static target_ulong calling_context[N_CC];
// static uint8_t cc_top = 255;

extern void afl_maybe_log_call(abi_ulong, abi_ulong);
extern void afl_maybe_log_edge(abi_ulong, abi_ulong);
extern void afl_maybe_log_ma(abi_ulong, abi_ulong, int);
extern void afl_maybe_log_mw(abi_ulong, abi_ulong, abi_ulong);
extern void afl_maybe_log_brcond(abi_ulong, abi_ulong, abi_ulong, abi_ulong);


void helper_update_calling_context(target_ulong pc, target_ulong target)
{
    // afl_maybe_log_call(target, calling_context[cc_top]);
    // cc_top++;
    // calling_context[cc_top] = (target >> 4) ^ (target << 8);

    // afl_maybe_log_call(target, calling_context);
    afl_maybe_log_call(target, pc);
    calling_context ^= ((pc >> 4) ^ (pc << 8));
...
void helper_maybe_log_edge(target_ulong pc, target_ulong from)
{
    afl_maybe_log_edge(pc, from);    
}

void helper_maybe_log_ma(target_ulong mem_loc, target_ulong from, int rw)
{
    afl_maybe_log_ma(mem_loc, from, rw);
}

void helper_maybe_log_mw(target_ulong pc, target_ulong mem_loc, target_ulong val){
    afl_maybe_log_mw(pc, mem_loc, val);
}


void helper_maybe_log_brcond(target_ulong pc, target_ulong cond, target_ulong c1, target_ulong c2){
    afl_maybe_log_brcond( pc, cond, c1, c2);
}
