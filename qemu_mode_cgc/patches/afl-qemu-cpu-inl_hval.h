...
static unsigned int afl_inst_rms = MAP_BIT_SIZE;
...
static void afl_setup(void) {
  ...
  if (inst_r) {
    ...
    afl_inst_rms = MAP_BIT_SIZE * r / 100;
...
#define N_GRAM (1 << (N_QUEUES - 1))
#define N_BLOCK ( N_GRAM * 2)
...
void afl_maybe_log(abi_ulong next_pc, abi_ulong cur_loc) {
  ...
  static abi_ulong n_block[N_BLOCK];
  /* Optimize for cur_loc > afl_end_code, which is the most likely case on
     Linux systems. */

  if (((cur_loc > afl_end_code || cur_loc < afl_start_code) && 
       (next_pc > afl_end_code || next_pc < afl_start_code)) || !afl_area_ptr)
    return;
  ...
  cur_loc &= MAP_BIT_SIZE - 1;

  next_pc = (next_pc >> 4) ^ (next_pc << 8);
  next_pc &= MAP_BIT_SIZE - 1;
  ...
  uint64_t* afl_trace_p = (uint64_t*)afl_area_ptr;
  unsigned char *afl_bitmap = afl_area_ptr + 8;
  ...
  for(i = 0; i < N_BLOCK - 2; i++){
    n_block[i] = n_block[i + 2]; //?
  }
  n_block[N_BLOCK - 2] = cur_loc;
  n_block[N_BLOCK - 1] = next_pc;
  // queue 1, n1
  acc = 0;
  base = N_BLOCK - 2;
  for(i = 0; i < 2; i += 2){
    e = (n_block[base + i] >> 1) ^ n_block[base + i + 1]; 
    acc ^= e;
  }
  acc &= (MAP_BIT_SIZE - 1);
  afl_bitmap[acc >> 3] |= (1 << (acc & 7));
  ...
  base = N_BLOCK - 8;
  for(i = 0; i < 8; i += 2){
    e = n_block[base + i] ^ (n_block[base + i + 1] << 1);
    acc ^= e;
  }
  acc &= (MAP_BIT_SIZE - 1);
  acc += (MAP_BIT_SIZE * 2);
  ...
  base = N_BLOCK - 32;
  for(i = 0; i < 32; i += 2){
    e = (n_block[base + i] >> 1) ^ (n_block[base + i + 1] << 1);
    acc ^= e;
  }
  acc &= (MAP_BIT_SIZE - 1);
  acc += (MAP_BIT_SIZE * 4);
