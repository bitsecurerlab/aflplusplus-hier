...
// void afl_maybe_log(abi_ulong, abi_ulong, abi_ulong, abi_ulong);
...
void afl_maybe_log_edge(abi_ulong next_pc, abi_ulong cur_loc) {
  ...
  static abi_ulong n_edge[N_GRAM];
  ...
  n_edge[N_GRAM - 1] = (cur_loc ^ (next_pc >> 1));
...
void afl_maybe_log_call(abi_ulong target, abi_ulong site){

  if (((site > afl_end_code || site < afl_start_code) && 
       (target > afl_end_code || target < afl_start_code))) return;
  ...
  if (site >= afl_inst_rms || target >= afl_inst_rms) return;
