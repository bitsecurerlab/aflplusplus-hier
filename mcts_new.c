...
static inline void calc_score(mcts_tree_t* tree, mcts_node_t* node){
    ...
    double eweight = trace_map->weights[node->n_cov_level - 1];
    ...
    for(i = 0; i < (MAP_SIZE >> 3); i++){
        ...
        for(j = 0; j < 8; j++){
            ...
            d_score = ((double) 1.0) / cnt;
            // if(d_score < eweight) continue;

            if(val & (128 >> j)){
                // if(cnt < min_cnt1_p2){
                //     cnt_p2 = next_p2(cnt);
                //     if(cnt_p2 < min_cnt1_p2){
                //         min_cnt1_p2 = MAX(cnt_p2, th);
                //         score1 = 0;
                //         num1 = 0;
                //     }
                ...
                byte_val = fmap[k];
                if(byte_val < min_byte_val){
                    min_byte_val = byte_val;
                    min_byte = k;
                }
            }
            ...
            if(val1){
                // if(cnt < min_cnt2_p2){
                //     cnt_p2 = next_p2(cnt);
                //     if(cnt_p2 < min_cnt2_p2){
                //         min_cnt2_p2 = MAX(cnt_p2, th);
                //         score2 = 0;
                //         num2 = 0;
                //     }
                //     score2 += ((double) val1) / cnt;
                //     num2 += val1;
                // }
                score2 += d_score * val1;
                num2 += val1;
    ...
    // fprintf(stderr, "#%d min_cnt1_p2: %llu, score1: %f, num1: %llu, min_cnt2_p2: %llu, score2: %f, num2: %llu weight: %f, num3: %llu\n",
        // node->id, min_cnt1_p2, score1, num1, min_cnt2_p2, score2, num2, weight, num3);

    fprintf(stderr, "#%d score1: %f, num1: %llu, score2: %f, num2: %llu weight (%f): %f, num3: %llu\n",
        node->id, score1, num1, score2, num2, eweight, weight, num3);

    weight /= num3;
    node->base_score = (score1 / num1) * weight;
    node->fuzz_score = node->fuzz_score / 2 + (score2 / num2) * weight;    // #7

    node->min_fuzz_byte = min_byte;

    // if(tree->root->fuzz_level > 100)
        trace_map->weights[node->n_cov_level - 1] = weight;

}


static inline void adjust_score(mcts_tree_t* tree, mcts_node_t* leaf){
    mcts_node_t* cur = leaf;
    while(cur){
...
static mcts_node_t* best_child(mcts_tree_t* tree, mcts_node_t* node, double c_param){
    ...
    for(; kb_itr_valid(&itr); kb_itr_next(mcts, map, &itr)){
        p = &kb_itr_key(mcts_key_t, &itr);
        nx = p->node;
        double s1 = nx->base_score * 1;
        // double s2 = nx->fuzz_score / nx->fuzz_level;
        // double s2 = nx->fuzz_score * 2 * factor / (nx->fuzz_level + 1);
        double s2 = nx->fuzz_score * 1 / 2;
        u8 min_val = tree->trace_map->fuzz_map[MAP_SIZE * (nx->n_cov_level - 1) + nx->min_fuzz_byte];
        double s3 = factor * c_param * sqrt((double)nx->n_seeds / node->n_seeds) * sqrt( log((double)node->fuzz_level) / nx->fuzz_level ) / next_p2_lookup[min_val];
