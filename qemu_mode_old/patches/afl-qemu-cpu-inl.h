...
static unsigned char
               dummy[MAP_SIZE * AFL_N_MULTI_LEVEL_COV]; /* costs MAP_SIZE but saves a few instructions */
...
static unsigned char dummy_ctrl[1];
unsigned char      *afl_area_ctrl = dummy_ctrl;
__thread abi_ulong  afl_path_cksum;
u8                  afl_use_multi_level_cov;
...
void afl_setup(void) {
  ...
  if(getenv("AFL_USE_MULTI_LEVEL_COV")) { afl_use_multi_level_cov = 1; }
...
void afl_persistent_loop(void) {
  ...
  if (persistent_first_pass) {
    ...
    if (is_persistent) {
      ...
      u32 map_size;
      if (afl_use_multi_level_cov) map_size = MAP_SIZE * AFL_N_MULTI_LEVEL_COV; 
      else map_size = MAP_SIZE;
