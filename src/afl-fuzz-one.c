...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (!afl->use_hier_sched){  // not skip
...
#define FLIP_BIT(_ar, _b)                   \
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (!afl->non_instrumented_mode && (afl->stage_cur & 7) == 7) {
      ...
      u64 cksum = hash64(afl->fsrv.trace_bits_core, afl->fsrv.map_size, HASH_CONST);
          ...
          if (mutated_size > 0) {
            ...
            if (!el->afl_custom_fuzz_count) {
              ...
              if (afl->queued_paths != havoc_queued) {
                ...
                if (afl->n_cov_level == 1 && perf_score <= afl->havoc_max_mult * 100) {
