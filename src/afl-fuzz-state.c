    ...
    if (strncmp(env, "ALF_", 4) == 0) {
      ...
      while (match == 0 && afl_environment_variables[i] != NULL) {
        ...
        if (strncmp(env, afl_environment_variables[i],
          ...
          } else if (!strncmp(env, "AFL_USE_MULTI_LEVEL_COV",

                           afl_environment_variable_len)) {

            if(get_afl_env(afl_environment_variables[i])) {

                afl->n_cov_level = afl->fsrv.n_cov_level = AFL_N_MULTI_LEVEL_COV;
                afl->virgin_bits = ck_realloc(afl->virgin_bits, afl->fsrv.map_size * afl->n_cov_level);
                afl->map_tmp_buf = ck_realloc(afl->map_tmp_buf, afl->fsrv.map_size * afl->n_cov_level);
                afl->fast_cal = 1;
		            afl->afl_env.afl_cal_fast = 1;

            }

          } else if (!strncmp(env, "AFL_USE_HIER_SCHEDULE",

                           afl_environment_variable_len)) {

            if(get_afl_env(afl_environment_variables[i])) {

              afl->use_hier_sched = 1;
              afl->hier_sched = new_hier_sched(afl->fsrv.map_size);
              afl->n_tree_nodes = afl->hier_sched->tree->n_nodes;
              afl->n_tree_fuzzed_nodes = afl->hier_sched->tree->n_fuzzed_nodes;
              afl->n_tree_bits = afl->hier_sched->tree->n_bits;
              afl->n_tree_fuzzed_bits = afl->hier_sched->tree->n_fuzzed_bits;
      ...
      if(afl->n_cov_level > 1 && afl->use_hier_sched == 0){
        afl->top_rated = ck_realloc(afl->top_rated, afl->fsrv.map_size * sizeof(void *) * afl->n_cov_level);
      }
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  if(afl->hier_sched) { delete_hier_sched(afl->hier_sched); }
