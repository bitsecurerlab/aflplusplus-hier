...
u8 __attribute__((hot)) has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  if(virgin_map == afl->virgin_bits){  
    current = (u64 *)afl->fsrv.trace_bits;
    i = (afl->fsrv.map_size >> 3) * afl->n_cov_level;
  }
  else{
    current = (u64 *)afl->fsrv.trace_bits_core;
    i = (afl->fsrv.map_size >> 3);
  }
  ...
  if(virgin_map == afl->virgin_bits){
    current = (u32 *)afl->fsrv.trace_bits;
    i = (afl->fsrv.map_size >> 2) * afl->n_cov_level;
  }
  else{
    current = (u32 *)afl->fsrv.trace_bits_core;
    i = (afl->fsrv.map_size >> 2);
  }
...
void __attribute__((hot)) classify_counts(afl_forkserver_t *fsrv) {
  ...
  if(fsrv->n_cov_level > 1) { i *= (fsrv->n_cov_level - 1); }  //skip the last cov level
...
void __attribute__((hot)) classify_counts(afl_forkserver_t *fsrv) {
  ...
  if(fsrv->n_cov_level > 1) { i *= (fsrv->n_cov_level - 1); }
...
void minimize_bits(afl_state_t *afl, u8 *dst, u8 *src) {
  ...
  while (i < afl->fsrv.map_size * afl->fsrv.n_cov_level) {
...
u8 *describe_op(afl_state_t *afl, u8 hnb) {
  ...
  if (unlikely(afl->syncing_party)) {
    ...
    sprintf(ret, "sync:%s,src:%08u", afl->syncing_party, afl->syncing_case);

  } else {

    sprintf(ret, "src:%08u", afl->current_entry);

    if (afl->splicing_with >= 0) {

      sprintf(ret + strlen(ret), "+%08d", afl->splicing_with);
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (unlikely(afl->schedule >= FAST && afl->schedule <= RARE)) {
    ...
    cksum = hash64(afl->fsrv.trace_bits_core, afl->fsrv.map_size, HASH_CONST);
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    queue_fn = alloc_printf("%s/queue/id:%08u,%s", afl->out_dir,
                            afl->queued_paths, describe_op(afl, hnb));

#else

    queue_fn =
        alloc_printf("%s/queue/id_%08u", afl->out_dir, afl->queued_paths);
    ...
    if (cksum)
      ...
      afl->queue_top->exec_cksum =
          hash64(afl->fsrv.trace_bits_core, afl->fsrv.map_size, HASH_CONST);

    if (afl->use_hier_sched) { 
      update_feature_freq(afl->hier_sched, afl->fsrv.trace_bits);
      do_clustering(afl->hier_sched, afl->queue_top, afl->fsrv.trace_bits); 
    }
...
#ifdef WORD_SIZE_64
        simplify_trace(afl, (u64 *)afl->fsrv.trace_bits_core );
#else
        simplify_trace(afl, (u32 *)afl->fsrv.trace_bits_core );
#endif                                                     /* ^WORD_SIZE_64 */
