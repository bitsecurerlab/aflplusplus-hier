...
void write_stats_file(afl_state_t *afl, double bitmap_cvg, double stability,
  ...
  u32   t_bytes = count_non_255_bytes(afl, afl->virgin_bits_core);
  u8    fn[PATH_MAX];
  FILE *f;
  ...
  fprintf(f,
          ...
          "tree_bits         : %u %u %u\n"
          "tree_fuzzed_bits  : %u %u %u\n"
          "time_log          : %llu %llu %llu\n"
          "edges_found       : %u\n"
          "var_byte_count    : %u\n"
          "havoc_expansion   : %u\n"
          "afl_banner        : %s\n"
          ...
          -1,
#endif
          afl->n_tree_nodes[1], afl->n_tree_nodes[2], afl->n_tree_nodes[3],
          afl->n_tree_fuzzed_nodes[1], afl->n_tree_fuzzed_nodes[2], afl->n_tree_fuzzed_nodes[3],
          afl->n_tree_bits[1], afl->n_tree_bits[2], afl->n_tree_bits[3],
          afl->n_tree_fuzzed_bits[1], afl->n_tree_fuzzed_bits[2], afl->n_tree_fuzzed_bits[3],
          afl->hier_sched->t[0] / 1000, afl->hier_sched->t[1] / 1000, afl->hier_sched->t[2] / 1000,
...
void maybe_update_plot_file(afl_state_t *afl, double bitmap_cvg, double eps) {
  ...
  fprintf(
      afl->fsrv.plot_file,
      "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %u, %u, %u, %u, %u, %u, %u %u %u, %u %u %u, %llu\n",
...
#define IB(i) (val_buf[(i)])
  ...
  t_bytes = count_non_255_bytes(afl, afl->virgin_bits_core);
  ...
  if (cur_ms - afl->stats_last_plot_ms > PLOT_UPDATE_SEC * 1000) {
    ...
    maybe_update_plot_file(afl, t_bytes, afl->stats_avg_exec);
  ...
  t_bits = (afl->fsrv.map_size << 3) - 
            count_bits(afl, afl->virgin_bits + 
                            (afl->n_cov_level > 1 ? 1 : 0) * afl->fsrv.map_size * (COV_EDGE - 1));
