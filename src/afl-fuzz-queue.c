...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  /* skip trim in multi-level coverage mode */
  if(afl->n_cov_level > 1) q->trim_done = 1;
...
void update_bitmap_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  for (i = 0; i < afl->fsrv.map_size * afl->fsrv.n_cov_level; ++i) {
    ...
    if (afl->fsrv.trace_bits[i]) {
      ...
      if (!q->trace_mini) {
        ...
        u32 len = (afl->fsrv.map_size >> 3) * afl->fsrv.n_cov_level;
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (afl->use_hier_sched) { perf_score = MAX(perf_score / 10, 10U); }
