...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  if (q->exec_cksum) {
    ...
    memcpy(afl->first_trace, afl->fsrv.trace_bits_core, afl->fsrv.map_size);
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    afl->fsrv.trace_ctrl[0] = (q->exec_cksum ? 1 : 0);
    ...
    if (!afl->non_instrumented_mode && !afl->stage_cur &&
        !count_bytes(afl, afl->fsrv.trace_bits_core)) {
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < afl->fsrv.map_size; ++i) {
          ...
          if (unlikely(!afl->var_bytes[i]) &&
              unlikely(afl->first_trace[i] != afl->fsrv.trace_bits_core[i])) {
  ...
  q->bitmap_size = count_bytes(afl, afl->fsrv.trace_bits_core);
  q->handicap = handicap;
  q->cal_failed = 0;
...
void sync_fuzzers(afl_state_t *afl) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    u8 entry[12];
    sprintf(entry, "id:%08u", next_min_accept);
    while (m < n) {
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  // if (afl->use_hier_sched) { update_feature_freq(afl->hier_sched, afl->fsrv.trace_bits); }
